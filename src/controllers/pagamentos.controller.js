...
export async function listarPagamentos(req, res) {
  try {
    ...
    res.json({ data: pagamentos, total, page: Number(page), limit: Number(limit) })
  } catch (err) {
...
export async function resumoPagamentos(req, res) {
  try {
    const [pago, pendente] = await Promise.all([
      prisma.pagamento.aggregate({ where: { estado: 'pago' },     _sum: { valor: true }, _count: true }),
      prisma.pagamento.aggregate({ where: { estado: 'pendente' }, _sum: { valor: true }, _count: true }),
    ])
...
export async function criarPagamento(req, res) {
  try {
    const { alunoId, sessaoId, valor, metodo, estado, dataPagamento, descricao } = req.body

    const pagamento = await prisma.pagamento.create({
      data: {
        alunoId, sessaoId, valor, metodo, estado, descricao,
